...
FlashMatrix easily scales to datasets with billions of data points and its
scalability is bound by the capacity of SSDs (Figure \ref{perf:fm} (b)).
When being executed on SSDs, FlashMatrix keeps most of the large matrices on
SSDs and thus leads to a very small memory footprint. The functional programming
interface generates a new matrix in each matrix operation, potentially leads
to high memory consumption. Owing to lazy evaluation,
