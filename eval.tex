...
Spark v2.0.1, H2O v3.14.2 and Revolution R Open v3.3.2.
...
\begin{figure}
  \vspace{-5pt}
	...
	on SSDs (FlashR-EM) compared with H2O and Spark MLlib. Correlation and GMM
	are not available in H2O. We run k-means and GMM on the PageGraph-32ev-sub
...
with MLlib, and 3 to 20 times performance gain when compared with H2O.
When running on SSDs, FlashR achieves at least half the speed of running in
memory. All implementations rely on BLAS for matrix multiplication. H2O
...
Spark MLlib on an EC2 cluster (Figure \ref{perf:cloud}). H2O recommends
...
Spark MLlib and H2O to process the datasets (PageGraph-32ev-sub and Criteo-sub).
Even though Spark MLlib and H2O has four times as much computation power as FlashR, FlashR
