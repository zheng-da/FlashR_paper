...
Most input data matrices are tall-and-skinny (TAS); they contain a large number of samples with a relatively
few features \cite{}. 
Even if the original data matrices have many features,
the first algorithmic step is usually dimension reduction \cite{}. 
Other wide-and-short (WAS) matrices contain a large number of features with
few samples \cite{}. Intermediate matrices and output matrices are usually small and may be square. 
We store vectors as a one-column dense matrix. 
Dense matrices can be stored physically in memory or on SSDs or represented virtually by a sequence of
computations.
...
groupby, output matrices
that have different partition size from the input matrices. We refer to these
matrices as \textit{sink matrices}. %The GenOps that
...
all virtual matrices matrix nodes need to have the same
