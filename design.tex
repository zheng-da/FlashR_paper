...
In data analysis, many data matrices contain a large number of samples with
a relatively small number of attributes, so the shape of the data matrices is
usually tall and skinny; if a data matrix has many attributes, the first step
is usually dimension reduction that results in a tall-and-skinny matrix.
...
on the partially aggregated results. For many aggregation operations such as
summation, the \textit{aggregate} and \textit{combine} functions are the same.
...
All of the GenOps take vectorized UDFs (VUDFs), which operates on a vector of
elements instead of an individual element. Most of the UDFs of the GenOps
operate on individual elements, which results in significant function call
...
\ref{sec:genop}. In addition, a VUDF type may have multiple forms to allow
GenOps to transform operations in VUDFs to increase the length of an input
vector and increase amortization of function call overhead.
\begin{itemize}
