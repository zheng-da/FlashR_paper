...
The example in Figure \ref{fig:kmeans}(a) computes an iteration of k-means
...
a few samples \cite{}. Intermediate matrices and output matrices tend to
...
the output matrices have a different partition size than the input matrices.  
\textit{Sink matrices} tend to be small
%.  These We refer to these virtual matrices as 
%The GenOps that
...
%In general, sink matrices are small 
...
The output of inner product of a wide matrix with a tall matrix is 
square in the small dimension of the inputs and tends to be small.
% because
...
computation to I/O.
Then it performs all computation for each input row to a DAG all together 
...
%and lazily evaluates operations within the DAG.
%GenOps and constructs a DAG automatically because most of the GenOps do not
%contain enough computation to achieve in-memory performance. 
...
\textit{partition dimension}. 
%\textit{Sink matrices} are edge nodes in the DAG.
%, because
...
the same thread, whcih minimizes
remote memory accesses in a NUMA machine.
...
% of the virtual matrix in the same thread. 
This helps ensure that a Pcache-partition resides in the CPU cache
when the next GenOp consumes it. 
In each thread, all intermediate matrices have only one 
Pcache-partitions materialized
...
We recycle memory buffers used for I/O and intermediate results.
