...
\caption{The architecture of FlashMatrix \rb{this can be shrunk so that the optimizer and SAFS layer are the same thickness.}}
\label{fig:arch}
\end{figure}
...
\end{minted}
\vspace{-5pt}
\caption{The R code of computing an iteration of k-means using GenOps. \rb{this looks bad.}}
\label{fig:kmeans}
\end{figure}
...
computations. Our treatment describes the data format for tall matrices. 
...
are tall and skinny. FlashMatrix supports both row-major and column-major
layouts (Figure \ref{fig:den_mat} (a) and (b)).  Supporting both formats
allows FlashMatrix to transpose data without a copy.
%FlashMatrix optimizes the generalized matrix operations on TAS dense matrices with tens of columns
%or fewer.
...
\ref{fig:den_mat}). All elements in an I/O-partition are stored
...
All GenOps output virtual matrices.
Virtual matrices are assembled to construct a directed
acyclic graph (DAG) that represents a matrix computation.
...
large as possible to increase the ratio of computation to I/O.
...
Figure \ref{fig:DAG} (a) shows a DAG for the k-means of Figure
...
A computation node references a \dz{GenOp} \rb{is this correct} and input matrices and
...
(Figure \ref{fig:DAG} (b)). To simplify evaluation and data flow, 
all virtual matrices in the internal matrix
nodes need to have the same \textit{long dimension}.  Matrices with 
different size in both dimensions must be an edge node in the DAG and will be materialized.
%, because
%any computation that uses these \textit{virtual matrices} cannot be connected
%to the same DAG.  
We refer to these as \textit{sink matrices}. 
% GenOps that aggregate, groupby, or compute inner products generate sink matrices.
...
In general, sink matrices are small and stored in memory in order 
to minimize data written to SSDs. 
The maximal size of the output matrix from an aggregation is $\sqrt{N}$ for $N$ elements in the input
...
FlashMatrix partitions matrices in a DAG in the long dimension and
materializes partitions separately (Figure \ref{fig:mater}). This is possible
because all matrices, except sink matrices, 
share the same long dimension. 
...
\rb{I think CPU partition is a bad term.  It makes it sound like it is being partitioned across mutliple CPUs.  Not the case.}
...
Upon materializing output to a sink matrix, the thread passes the output as input to the subsequent
DAG, instead of materializing the next CPU-partition in the same thread.
A CPU-partition is sufficiently small to fit in the CPU cache so that
the partition still resides in the CPU cache when the subsequent DAG consumes
