...
attributes, the first step is usually dimension reduction \cite{}, which
results in a tall-and-skinny matrix. FlashMatrix specifically
...
are applied to short-and-wide matrices.
...
in a DAG. The first type of GenOps, such as \textit{sapply} and \textit{mapply},
output matrices with the same \textit{long dimension} size as the input matrices.
The second type of GenOps, such as \textit{agg} and \textit{groupby}, output
matrices with different \textit{long dimension} sizes. The output matrices of the first
