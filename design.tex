...
FlashR. FlashR supports a small number of classes of generalized operations (GenOps)
...
\caption{Generalized operations (GenOps) in FlashR. $A$, $B$ and $C$ are
	...
	operates on elements of matrices.}
\vspace{-10pt}
\footnotesize
\begin{tabular}{|l|l|l|}
\hline
...
\hline
$C=groupby(A, f)$ & $C_{k}=f(A_{i,j}, C_{k})$,\\ & where $A_{i, j}=k$, over all $i,j$ \\
...
\hline
$C=cum.row(A, f)$ & $C_{i,j}=f(A_{i,j}, C_{i-1,j})$ \\
$C=cum.col(A, f)$ & $C_{i,j}=f(A_{i,j}, C_{i,j-1})$ \\
\hline
\end{tabular}
\normalsize
\label{tbl:genops}
...
When parallelizing matrix operations, FlashR aims at achieving good I/O
performance and load balancing as well as reducing remote memory access
in the NUMA architecture. As such, FlashR deploys a scheduler that dispatches
...
\begin{itemize}
	...
	\item For operations in Figure
		...
		thread and combine all partial results at the end of
	...
	\item For cumulative operations on a tall matrix in Figure \ref{fig:parallel}
		(j), a partition $i$ of the output matrix depends on
		a partition $i$ of the input matrix as well as a partition $i-1$
		of the output matrix. To reduce I/O and evaluate this operation with
		...
		that a partition $i$ depends on is ready, a thread generates the partition
		and moves it to the subsequent matrix operations. Otherwise, a thread moves
		to the next partition $i+1$ and gets its dependancy data ready. If the number
		of pending partitions reaches a threshold, a thread puts itself into sleep
		and waits for all dependancy data ready.
\end{itemize}
...
load data from external source, such as \textit{load.dense} and
...
output \textit{sink matrices}.
\textit{Sink matrices} tend to be small and once being
...
Internally, FlashR uses this mechanisum to lazily evaluate \textit{runif.matrix}:
FlashR generates random numbers when the matrix is used for the first time and
generated random numbers is cached so that the matrix returns the same
...
FlashR partitions matrices in a DAG and materializes partitions relatively
separately. This is possible because all matrices in a DAG except sink matrices
...
at run time.  Each thread materializes one Pcache-partition at a time from
...
FlashR further deploys multiple techniques to reduce CPU cache polution to
reduce data movement between CPU and memory. When evaluating the computations
in a DAG, a thread performs depth-first traversal and performs matrix operations
...
To further reduce CPU cache polution, FlashR discards a Pcache partition
once it is used by all subsequent matrix operations, and the memory buffer
of the Pcache partition are reused by the next matrix operation. As such,
the next matrix operation writes its output data in the memory that is
already in CPU cache. A Pcache partition can be read by multiple matrix
operations. As such, FlashR maintains a counter on each Pcache partition.
When the counter indicates the partition has been used by all subsequent
matrix operations, the Pcache partition is discarded.
