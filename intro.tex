...
is essential to move data efficiently both from
...
We overcome many challenges to move data from SSDs to CPU efficiently,
notably the large performance disparities between CPU and memory and between
memory and SSDs. The ``memory gap'' \cite{Wilkes91} continues to grow, with 
the difference between CPU and DRAM performance growing exponentially. 
% speed disparity of CPU and DRAM has increased exponentially
...
RAM outperformans SSDs by an order of magnitude for both latency and throughput.
%Even though the I/O
...
FlashMatrix by default materializes
only the output matrices (leaf nodes) of a DAG and keeps materialized results in
memory in order to minimize data written to SSDs. FlashMatrix streams
