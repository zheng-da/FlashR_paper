...
can deal with large-scale data analysis efficiently in a multicore
...
Previous SSD-based graph analysis frameworks \cite{flashgraph, gridgraph, graphene}
...
computation to I/O. FlashR evaluates all matrix operations in a DAG
in a single execution and performs two levels of matrix partitioning to
reduce data movement in the memory hierarchy. FlashR by default avoids
physically generating new large matrices and materializes only the output
matrices (leaf nodes) of a DAG, which are in generally small. FlashR streams
...
the same algorithms in H2O \cite{h2o} and Spark MLlib \cite{spark} by a factor
of $3-20$ in a large parallel machine with 48 CPU cores. In the Amazon cloud,
FlashR uses only one fourth of resources still matches or even outperforms H2O
and Spark MLlib, which suggests that FlashR is a much
