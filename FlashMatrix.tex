...
operations (GenOps) that require some functions to define the computation on
elements of the input matrices. It reimplements a large number of matrix
operations in the R framework with GenOps to execute R code in
parallel and out of core automatically. The functions passed to GenOps
are vectorized to reduce the overhead of function calls. In addition,
FlashMatrix fuses matrix operations to reduce data movement between CPU and
